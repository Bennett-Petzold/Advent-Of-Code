...
struct MemorySteppedPos<'a> {
    pub pos: Pos2D,
    pub steps: u64,
    pub memory: Cow<'a, Vec<Pos2D>>,
}

impl PartialOrd for MemorySteppedPos<'_> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for MemorySteppedPos<'_> {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.steps
            .cmp(&other.steps)
...
impl<I: IntoIterator<Item = S>, S> FillingMemSpace<I, S> {
    ...
    pub fn complete_path(
        ...
        let mut to_visit = BinaryHeap::from([Reverse(MemorySteppedPos {
            pos: Pos2D::new(0, 0),
            steps: 0,
            memory: Cow::Owned(truncated_path.to_vec()),
        })]);
        ...
        while to_visit.peek().map(|x| x.0.pos != final_pos) == Some(true) {
            ...
            let mut new_memory = top.memory.into_owned();
            new_memory.push(top.pos);
            let new_memory: Cow<Vec<_>> = Cow::Owned(new_memory);
