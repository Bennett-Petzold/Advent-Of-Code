...
fn main() {
    ...
    let input_file = BufReader::new(File::open(args().nth(3).unwrap()).unwrap())
        .lines()
        .map(|line| line.unwrap());
    let full_mem_space = FillingMemSpace::from_input(dim, input_file).unwrap();
...
fn part_2<I, S>(mut mem_space: FillingMemSpace<I, S>)
where
    ...
    <I as IntoIterator>::IntoIter: Sync,
{
    ...
    fn validate(&mut self, next_fill: Pos2D) -> Option<bool> {
        if let Some(overlap_idx) = self.path.iter().position(|entry| *entry == next_fill) {
            ...
            let new_paths = (1..self.path.len())
                .rev()
                .map(|test_idx| (&self.path[..test_idx], &self.path[test_idx..]))
                .chain(std::iter::once((&[] as &[_], &[] as &[_])))
                .par_bridge();

            if let Some(new_path) = new_paths.find_map_any(|(truncated_path, failed_prior)| {
                self.complete_path(truncated_path, failed_prior)
            }) {
                self.path = new_path;
                Some(true)
            } else {
